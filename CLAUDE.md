...
- **Voice (spoken replies):** Chatterbox TTS runs as a **native MPS host process** on `:8004` (Docker has no macOS GPU passthrough), managed as `stack.sh`'s 4th layer. Zero-shot cloning: a `.wav` in `voice/reference_audio/` *is* the voice (`VOICE_DEFAULT` in `config/voice.env`); Open WebUI's TTS points **directly** at `:8004/v1` (no bridge proxy). Install: `voice/setup.sh` (pinned repo + venv + MPS patch). Cloned real-person voices are personal-use only and never committed.
...
- **Safety holds through the UI:** the bridge's `--allowedTools` allowlist covers render/slice but **not** the printer; `print.sh`/Moonraker stays human-only. Never `--dangerously-skip-permissions`. `tests/test_bridge.py` enforces both.
- **The bridge is authenticated.** `/v1/*` requires `Authorization: Bearer $BRIDGE_TOKEN` (generated by `setup.sh`; Open WebUI sends it as its OpenAI key). Binding loopback is NOT an alternative — the container reaches the host via `host.docker.internal`, which never arrives on loopback. See `SECURITY.md`.

## Branches and the promotion gate
`main` is what the public clones; **`dev` is where work lands.** Never commit straight
to `main` — it is only ever fast-forwarded from a `dev` that has passed the gate.

The gate has two halves, because CI cannot run the real stack (GitHub runners have no
Colima, no Apple MPS, no Claude subscription):
- **Headless / CI** (`.github/workflows/ci.yml`) — `pytest tests/`, shellcheck, compose
  validation, dependency resolution.
- **Local, on real hardware** — `scripts/stack.sh check --deep`, which is the
  authoritative end-to-end check: container, bridge, skin, wiring, the 401 auth
  assertion, real speech synthesis and a real chat round-trip.

`tests/test_bridge.py` guards the **print gate**: `allowed_tools()` must never expose
`print.sh` or a network fetch tool. If a change makes those tests fail, the allowlist
regressed — do not edit the test to match.
...
- `voice/` — TTS layer: `setup.sh` (pinned engine install), `make-reference.sh` (recording → voice), `reference_audio/` (voices; `.wav` gitignored), `server/` (vendored, gitignored)
- `config/voice.env` (tracked) — voice/port wiring: `VOICE_ENABLED` · `VOICE_PORT` · `VOICE_DEFAULT` · `WEBUI_PORT` · `BRIDGE_PORT`
