...
- 2026-06-29 — **DEFERRED: bridge still writes to `out/preview/`.** This round established the per-project convention but intentionally left the web bridge untouched so the working UI didn't break. The bridge (`bridge/app.py`: `PREVIEW_DIR`/`SCAD_DIR`/glob/`/files` mount) and `render.sh`'s default outdir still use `out/preview/`, so parts built *through the chat UI* are only partially co-located. → **Next step:** rewire the bridge to read/write/serve `projects/<slug>/` (URLs `/files/<slug>/<file>`), repoint `render.sh`'s default, then update `bridge/README.md` + `find-model` download dir. Re-verify with `scripts/stack.sh check --deep`.
